...
void AvlTree<Comparable>::remove(const Comparable & x, AvlNode<Comparable> * & t) const
{
  if( t == NULL ) // never found x
    ...
    if( x < t->element )
      ...
      else // Found x
      {
        ...
        {
          AvlNode<Comparable> * temp = NULL; // default is being a leaf
  ...
  if(t)
	...
	template <class Comparable>
        void AvlTree<Comparable>::tree2queue( AvlNode<Comparable> *t ) 
        {
	...
	const Queue<AvlNode<Comparable > * > AvlTree<Comparable>::queuereturn() const
	{
	...
	template <class Comparable>
        void AvlTree<Comparable>::removequeue( ) 
	{
		while(q.isFull())
		q.dequeue();
	}
